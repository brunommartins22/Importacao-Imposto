...
//                JPAUtil.executeCreateScript();
                Connection connection = JPAUtil.getEntityManager().unwrap(SessionImpl.class).connection();
                Database database = DatabaseFactory.getInstance().findCorrectDatabaseImplementation(new JdbcConnection(connection));
                Liquibase liquibase = new Liquibase("liquibase/updateDB.xml", new ClassLoaderResourceAccessor(), database);
            ...
            worker.addPropertyChangeListener((evt) -> {
                if (evt.getPropertyName().equals("state")) {
                    if (evt.getNewValue() == SwingWorker.StateValue.DONE) {
                        JfrmPrincipal jp = new JfrmPrincipal();
                        dialog.dispose();
                        jp.setVisible(true);
